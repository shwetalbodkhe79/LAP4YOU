...
exports.checkOut = async (req, res) => {
  try {
    ...
    if (req.body.paymentMethod === "COD") {
      ...
      const options = {
        amount: orderDetails.finalPrice * 100, // paise
        currency: "INR",
        receipt: String(transactionID), // convert number to string
      };
...
exports.verifyPayment = async (req, res) => {
  try {
    ...
    if (razorpay_signature === expectedSign) {
      ...
      // reduce stock
      for (const product of req.session.orderDetails.summary) {
        await productCLTN.updateOne(
          {
            _id: product.product,
            "RAMSSD.price": product.totalPrice / product.quantity,
          ...
          {
            $inc: { "RAMSSD.$.quantity": -1 * product.quantity },
          }
        );
      }
